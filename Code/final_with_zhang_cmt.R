    ...
    fluidPage(
      ...
      fluidRow(
        column(12, align = "center",
               ...
               actionButton("reset_graph", "Reset Graph", class = "btn btn-secondary")
        )
      ),
      br(),
      fluidRow(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$delete_node, {
    ...
    if (is.null(selected_node) || selected_node == "") {
      showModal(modalDialog(
        ...
        "Please select a node first before deleting.",
        easyClose = TRUE,
    ...
    showModal(modalDialog(
      ...
      paste("Are you sure you want to delete node:", selected_node, "?"),
      easyClose = FALSE,
      footer = tagList(
        modalButton("Cancel"),
        actionButton("confirm_delete_node", "Delete", class = "btn btn-danger")
      )
    ))
  })
  ...
  observeEvent(input$confirm_delete_node, {
    ...
    if (!is.null(selected_node) && selected_node != "") {
      ...
      updateSelectInput(session, "graph_selected", choices = rv$nodes$label, selected = "")
      create_graphicaltesting_objects()
      rv$gt_object <- NULL
      rv$gt_log <- ""
      rv$gt_summary <- NULL
    ...
    visNetwork(gnodes, gedges) %>%
      visNodes(
        ...
        color = list(
          ...
          border = "blue",
          highlight = list(background = "yellow", border = "orange")  # Add this line
        ),
  ...
  output$edge_table <- renderDataTable({
    ...
    display_data <- rv$edges[, cols, drop = FALSE]
    colnames(display_data) <- c("From", "To", "Weight")
    display_data
  }, options = list(dom = 't', paging = FALSE, destroy = TRUE), rownames = FALSE)
